...
import { Segment, Track, Video } from '~/types/draff.json';

dotenv.config();

const root_video_directory = process.env.ROOT_VIDEO_DIRECTORY || 'D:/Kien/videos';


function generateJsonFile(draffFilePath: string, jsonFilePath: string, folderResult: FolderResult[]) {
    // Read the draff file
    const draffContent = fs.readFileSync(draffFilePath, 'utf-8');

    // Parse the draff content to JSON
    const jsonContent = JSON.parse(draffContent);

    const videos = createVideoObject(folderResult);

    jsonContent.materials.videos = videos;
...
function createVideoObject(folderResult: FolderResult[]): Video[] {
    const videos: Video[] = [];
    folderResult.forEach((result) => {
        const video: Video = {
            "aigc_history_id": "",
            "aigc_item_id": "",
            "aigc_type": "none",
            ...
            "object_locked": null,
            "origin_material_id": "",
            "path": join(root_video_directory, result.folder, result.video).replaceAll("\\", "/"),
            "picture_from": "none",
...
function createTrackObject(folderResult: FolderResult[], videos: Video[]): Track[] {
    ...
    folderResult.forEach((result) => {
        const segment: Segment = {
            ...
            "material_id": videos[index].id, // video id
            "raw_segment_id": "",
            "render_index": 0,
            "render_timerange": {
                "duration": 0,
